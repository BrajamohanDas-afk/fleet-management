...
export default function PublicShare() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => getPublicLocation(token),
  ...
  if (error || !data) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6 text-center text-red-700">
        This sharing link is invalid, revoked, or expired.
      </div>
    );
  }

  const hasLocation = data.latitude != null && data.longitude != null;
  const position: FleetPosition = {
    ...data,
    vehicle_type: data.vehicle_type,
    status: data.status as FleetPosition['status'],
    updated_at: data.received_at ?? data.expires_at,
    device_id: null,
  };
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4">
        <div className="rounded-xl bg-white p-5 shadow-sm">
          <h1 className="text-xl font-bold text-slate-900">
            Live location - {data.registration_no}
          </h1>
          <p className="text-sm text-slate-500">
            {data.vehicle_code} - Last update{' '}
            {data.received_at
        ...
        <div className="min-h-[70vh] flex-1">
          {hasLocation ? (
            ...
            <div className="flex h-full min-h-[70vh] items-center justify-center rounded-xl bg-white p-6 text-center shadow-sm">
              <div>
